...
class VoiceRoomViewController: VRBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        self.isOwner = user.uid == owner.uid
        self.local_index = self.isOwner ? 0 : nil
        self.vmType = self.getSceneType(type)
...
extension VoiceRoomViewController {
    ...
    func loadKit() {
        ...
        self.rtckit.setClientRole(role: isOwner ? .owner : .audience)
    ...
    func fetchRTCToken() {
        guard let room_id = self.roomInfo?.room?.room_id else {return}
        guard let channel_id = self.roomInfo?.room?.channel_id else {return}
        VoiceRoomBusinessRequest.shared.sendGETRequest(api: .fetchRTCToken(roomId: room_id, channel_id: channel_id), params: [:]) { result, error in
            if error == nil,let rtc_token = result?["rtc_token"] as? String {
                self.token = rtc_token
                //加载RTC+IM
                self.loadKit()
            }
        }
    ...
    func layoutUI() {
        ...
        bgImgView.image = UIImage(named: "bg_ease")
