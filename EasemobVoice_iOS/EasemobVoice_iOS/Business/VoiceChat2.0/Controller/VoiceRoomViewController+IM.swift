...
extension VoiceRoomViewController: VoiceRoomIMDelegate {
    ...
    private func updateMic(_ map: [String : String]?, fromId: String) {
        ...
        for key in keys {
            ...
            if !self.isOwner {
                ...
                if mic.member?.uid ?? "" == VoiceRoomUserInfo.shared.user?.uid ?? "" {
    ...
    func sendTextMessage(text: String) {
        ...
        self.showMessage(message: EMChatMessage(conversationID: roomId, body: EMTextMessageBody(text: text), ext: ["userName":VoiceRoomUserInfo.shared.user?.name ?? ""]))
    ...
    func reLogin() {
        VoiceRoomBusinessRequest.shared.sendPOSTRequest(api: .refreshIMToken(()), params: ["deviceId":UIDevice.current.deviceUUID,"uid":VoiceRoomUserInfo.shared.user?.uid ?? "","token":VoiceRoomUserInfo.shared.user?.im_token ?? ""]) { [weak self] result, error in
    ...
    func showMessage(message: EMChatMessage) {
        if let body = message.body as? EMTextMessageBody,let userName = message.ext?["userName"] as? String {
