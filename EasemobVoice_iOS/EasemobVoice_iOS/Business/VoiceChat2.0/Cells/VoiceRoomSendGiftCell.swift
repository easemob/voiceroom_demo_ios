...
public class VoiceRoomSendGiftCell: UICollectionViewCell {
    ...
    lazy var icon: UIImageView = {
        UIImageView(frame: CGRect(x: self.contentView.frame.width/2.0-24, y: 16.5, width: 48, height: 48)).contentMode(.scaleAspectFit)
    }()
    
    lazy var name: UILabel = {
        UILabel(frame: CGRect(x: 0, y: self.icon.frame.maxY+4, width: self.contentView.frame.width, height: 18)).textAlignment(.center).font(.systemFont(ofSize: 12, weight: .regular)).textColor(UIColor(0x040925))
    }()
